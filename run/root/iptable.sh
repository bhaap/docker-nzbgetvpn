...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# setup route for nzbget webui using set-mark to route traffic for port 61473 to eth0
	echo "61473    webui" >> /etc/iproute2/rt_tables
...
for lan_network_item in "${lan_network_list[@]}"; do
	...
	iptables -A INPUT -i eth0 -s "${lan_network_item}" -p tcp --dport 61473 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# accept output from nzbget webui port 61473 - used for external access
	iptables -t mangle -A OUTPUT -p tcp --dport 61473 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 61473 -j MARK --set-mark 1
...
for lan_network_item in "${lan_network_list[@]}"; do
	...
	iptables -A OUTPUT -o eth0 -d "${lan_network_item}" -p tcp --sport 61473 -j ACCEPT
